    ...
    $.widget("ui.selectit", {
        ...
        _create: function() {
                    ...
                    if (this.inline) {
                        if (this.previousFocusedItem!==null) {
                            try {
                                ...
                                if (this.previousFocusedItem.data("ui-selectit-item").selected) {
            ...
            this.menuwrapper = $("<div>")
                    .addClass("ui-selectit-menu ui-front")
                    .append(this.menu.element)
                    .append($("<div>")
                        .addClass("ui-selectit-header"))
                    .append($("<div>")
                        .addClass("ui-selectit-footer"))
                    .appendTo(this._appendTo())
                    .hide();
            this.menuwrapperVisible = false; //Store visibility of menu externally.  The is(":visible") call is time consuming.
            this.inline = false; //This indicates if the selectit is currently in "inline" mode
             ...
             * to blur the "hidden" button when leaving an inline selectit.
             */
            ...
            this._on(this.menu.element, {
                ...
                mousedown: function(event) {
                     ...
                     * the user clicks somewhere outside of the selectit.
                ...
                menufocus: function(event, ui) {
                    ...
                    var option = ui.item.data("ui-selectit-item");
                    ...
                    if (ui.item.hasClass("ui-state-disabled")) {
                        ...
                        if (that.previousFocusedItem!==null) {
                            try {
                                if (that.previousFocusedItem.data("ui-selectit-item").selected) {
                        ...
                        if (false === this._trigger("focus", event, {item: option})) {
                            this.liveRegion.html(this._getLabel($(option)));
                        }
                    }
                },
        ...
        _setOptions: function(options) {
            ...
            $.each( options, function( key, value ) {
                if ( key === "inline" ) {
                    ...
                    if (value==="" && this.options.buttonClass) {
                        this.buttonlabel.removeClass(this.options.buttonClass);
                    } else {
                        this.buttonlabel.addClass(this.options.buttonClass);
                    }
        ...
        _selectItem: function(li, selected) {
            ...
            if (selected) {
                icon.removeClass("ui-selectit-nocheck")
            ...
            if (this.menuCreated) {
                ...
                if ($.type(li)==="undefined") {
                    ...
                    $.each(children, function(indx, elem) {
                        elem = $(elem);
                        if (elem.data("ui-selectit-item")===option) {
                            li = elem;
                            return false;
                        }
                    });
                ...
                if (this.multiple) {
                    if (!option.selected) {
                        for (var ii=0;ii<this.selectedMenuItems.length;ii++) {
                            if (this.selectedMenuItems[ii].data("ui-selectit-item")===option) {
        ...
        _multiSelect: function(siblings, selected, start, end) {
            ...
            for (var ii=start;ii<=end;ii++) {
                ...
                var optSibling = liSibling.data("ui-selectit-item");
                    ...
                    if (optSibling.selected!==selected && !liSibling.hasClass("ui-state-disabled")) {
                        ...
                        if (selected) {
                            ...
                            for (var jj=0;jj<this.selectedMenuItems.length;jj++) {
                                if (this.selectedMenuItems[jj].data("ui-selectit-item")===optSibling) {
		...
		_getLabel: function(option) {
			...
			return $.type(label)!=="undefined" ? label : option.text();
		},
        ...
        _update: function(updateMenu) {
            ...
            var html = this.buttonhidden.html(function() {
                if ( selected.length === 0 ) {
                    ...
                    return that._getLabel(selected);
                }
            }).html();
        ...
        refresh: function() {
            ...
            if (this.options.inline===true && !this.inline) {
                this.menuwrapper.appendTo(this.buttonwrapper).addClass("ui-selectit-inline").removeClass("ui-front").css({
                    position: "",
                    top: "",
                    left: "",
                    display:""
                ...
                this.inline = true;
                this.buttonlabel.addClass("ui-selectit-hidden");
                this._open();
            } else if (this.options.inline!==true && this.inline) {
                this.menuwrapper.appendTo(this._appendTo()).removeClass("ui-selectit-inline").addClass("ui-front");
                this.inline = false;
                this.buttonlabel.removeClass("ui-selectit-hidden");
                this.close();
            }
            ...
            allOptions.each(function(i, e) {
                label[i] = "<br>" + $("<div>").append(that._updateButton($("<span>"), that._getLabel($(e)))).html();
            });
            ...
            if (images.length!==0) {
				...
				this.imagesInMenu = images.length;
				var f = function() {
					$(this).off("load error");
					that.imagesInMenu--;
					if (that.imagesInMenu === 0) {
						that.calculateAndResizeButton = true;
						that._calculateAndResizeButton();
					}
				};
				images.one({
					load: f,
        ...
        _calculateAndResizeButton: function() {
            ...
            if (this.calculateAndResizeButton) {
            	//Determine menu width offset
            	var pre,post;
            	if (!this.menuhiddensingle.hasClass("ui-state-default")) {
	            	pre = Math.ceil(this.menuhiddensingleli.width())+1;
	            	this.menuhiddensingle.addClass("ui-state-default");
	            	post = Math.ceil(this.menuhiddensingleli.width())+1;
            	} else {
	            	post = Math.ceil(this.menuhiddensingleli.width())+1;
	            	this.menuhiddensingle.removeClass("ui-state-default");
	            	pre = Math.ceil(this.menuhiddensingleli.width())+1;
            	}
            	this.optimalButtonSize = post;
            	if (this.multiple) {
            		this.menuhiddensingleli[0].innerHTML = "XXX Options Selected";
            		this.menuhiddensingle.addClass("ui-state-default");
	                this.optimalButtonSize = Math.ceil(this.menuhiddensingleli.width())+1;
            	}
            	this.menuWidthOffset = post - pre;
        ...
        _createMenu: function() {
            ...
            if (this.multiple) {
                ul.removeClass( "ui-selectit-single" );
            } else {
        ...
        _resizeMenu: function() {
            if (this.menuwrapperVisible) {
                ...
                if (this.options.popup===true) {
                    ...
                    }).addClass("ui-selectit-popup ui-corner-all");
                    var topPad = lis.first();
                    ...
                    this.menuwrapper.removeClass("ui-selectit-popup ui-corner-all")
                            .css({
                                "overflow-y": "",
                                "height": "",
                ...
                //Apply width offset
                if (this.menuWidthOffset!==undefined && this.menuWidthOffset>0) {
                    this.menu.element.width(this.menu.element.width()+this.menuWidthOffset);
                }
        ...
        _organizeOptions: function(items) {
            ...
            $.each(items, function(i, item) {
                ...
                var label = that._getLabel($(item)).replace(/^\s+|\s+$/g, '');
                if (label.length===0) {
                    label = "&nbsp;";
                }
        ...
        _renderItem: function(ul, item, disabled, indent, selectedLI) {
            ...
            if ( $.type(item.optionGroups)==="undefined" ) {
                li = $("<li>")
                        ...
                        .appendTo(ul)
                        .data( "ui-selectit-item", item.option);
        ...
        _indentItem: function(item, indent) {
            var span = $("<span>").html(this._getLabel($(item)));
            for (var i=0;i<indent;i++) {
                span = $("<span>")
                        .addClass( "ui-selectit-indent" )
                        .append(span);
            }
            return span;
        },
